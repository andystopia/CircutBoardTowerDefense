...
public static class GameObjectHelper
{
    ...
    public static void AssertOnlyComponentOfType<T>(MonoBehaviour obj) where T : class
    {
        ...
        if (objT == null)
            Debug.LogAssertion(
                $"Passed object to ${nameof(AssertOnlyComponentOfType)}<${nameof(T)}> does not implement interface.");
        Assert.AreEqual(objT, obj.GetComponent<T>(),
            $"Warning game behavior is not the only instance of interface ${nameof(T)} on the object. ",
            new ReferenceEq<T>());
    }
